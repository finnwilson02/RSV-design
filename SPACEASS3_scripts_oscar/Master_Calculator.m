...
fprintf('BURN 1.1: delta-v = %f m/s, location: %s \n', delta_v1_M1, 'orbit B1 apogee');
fprintf('BURN 1.2: delta-v = %f m/s, location: %s \n', delta_v2_M1, 'orbit B1 apogee');
fprintf('TIME TAKEN: %f hrs\n', M1_time_elapsed);

% increment
tElapsed = tElapsed + M1_time_elapsed;
dVsum = dVsum + abs(delta_v1_M1) + abs(delta_v2_M1);
fprintf('RUNNING TOTAL DELTA-V: %f m/s \n', dVsum);
fprintf('RUNNING TOTAL ELAPSED TIME: %f hrs \n', tElapsed);
fprintf('\n');

fprintf('NET TOTALS (INCLUDING MANOEUVRE 1)\n');
fprintf('TOTAL DELTA-V = %f m/s\n', dVsum);
fprintf('TOTAL ELAPSED TIME = %f hrs\n\n', tElapsed);

fprintf('FOR %2.0f SERVICES:\n', serviceCount);
fprintf('DELTA-V NEEDED: %f m/s\n', dVsum*serviceCount);
fprintf('NET SERVICE TIME: %f days\n', tElapsed*serviceCount/24);
